...
func New(w io.Writer) *Alog {
	...
	return &Alog{
		...
		// create mutex with pointer to that
		m: &sync.Mutex{},
...
func (al Alog) Start() {
	wg := &sync.WaitGroup{}
	loop:
	// check if there is a message coming in
	for {
		select {
			case	msg := <-al.msgCh:
				wg.Add(1)
				// create go routine and pass message in
				go al.write(msg, wg)
			case <-al.shutdownCh:
				wg.Wait()	
				al.shutdown()
...
func (al Alog) write(msg string, wg *sync.WaitGroup) {
	// use mutex to ensure one message being written at one time
	defer wg.Done()
	al.m.Lock()
	defer al.m.Unlock()
